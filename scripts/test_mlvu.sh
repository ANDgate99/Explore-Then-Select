...
if [[ $METHOD == llava* ]]; then
    echo "Using llava"
    CUDA_VISIBLE_DEVICES=$USE_CUDA python -m test.test_mlvu_llava \
        --frames $FRAMES \
    ...
    echo "Using qwen2 vl"
    CUDA_VISIBLE_DEVICES=$USE_CUDA python -m test.test_mlvu_qwen \
        --frames $FRAMES \
        --budget_frames $BUDGET_FRAMES \
        --method $METHOD \
        --start $START \
    ...
    CUDA_VISIBLE_DEVICES=$USE_CUDA python -m test.test_mlvu_qwen_2_5 \
        --frames $FRAMES \
        --budget_frames $BUDGET_FRAMES \
        --method $METHOD \
        --start $START \
